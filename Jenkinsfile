pipeline {
    ...
    parameters {
        ...
        string(name: 'APP_FILTER', defaultValue: '', description: 'Optional: comma/space-separated app names to limit within the release (e.g. "HRT-API,CUSTOMER-API"). Blank = all apps in the release.')
    }
    environment {
        GMU_HOME = 'C:\\gmu'
            ...
            steps {
                script {
                    ...
                    if (releaseInput == 'ALL') {
                        ...
                        releasesToRun = releaseInput.split('[,\\s]+').collect { it.trim() }.findAll { it && fileExists("releases/${it}/manifest.yaml") }
                    }
                    if (releasesToRun.isEmpty()) {
                        error "No valid releases found for RELEASE=${params.RELEASE}"
                    }

                    def appFilter = params.APP_FILTER?.trim() ? params.APP_FILTER.split('[,\\s]+').collect { it.trim() }.findAll { it } : []
                    logReleaseApiMap = [:]
                ...
                script {
                    logReleaseApiMap.each { release, apps ->
                        apps.each { app ->
                            ...
                            if (!fileExists(apiDir)) { error "API directory not found: ${apiDir}" }
                            if (!fileExists("${apiDir}/mappings.xml")) { error "mappings.xml missing in ${apiDir}" }
                            if (!fileExists("${apiDir}/dependencies.xml")) { error "dependencies.xml missing in ${apiDir}" }
                            if (!fileExists("${apiDir}/rootFolder")) { error "rootFolder directory missing in ${apiDir}" }
                            if (!fileExists("${apiDir}/dependencies")) { error "dependencies directory missing in ${apiDir}" }
                            def rootFiles = findFiles(glob: "${apiDir}/rootFolder/**/*")
                            if (rootFiles.length == 0) { error "rootFolder is empty for ${app}" }
                ...
                script {
                    logReleaseApiMap.each { release, apps ->
                        apps.each { app ->
                            ...
                            echo "Preparing NewOrUpdate mappings for ${app}"

                            int policyExitCode = bat(returnStatus: true, script: """
                                @echo off
                                call "%GMU_HOME%\\GatewayMigrationUtility.bat" manageMappings -b "${currentBundlePath}" -t POLICY -a NewOrUpdate
                            """)
                            if (policyExitCode != 0) { error "Failed to apply NewOrUpdate mapping for POLICY in ${app}. Exit code: ${policyExitCode}" }

                            int serviceExitCode = bat(returnStatus: true, script: """
                                @echo off
                                call "%GMU_HOME%\\GatewayMigrationUtility.bat" manageMappings -b "${currentBundlePath}" -t SERVICE -a NewOrUpdate
                            """)
                            if (serviceExitCode != 0) { error "Failed to apply NewOrUpdate mapping for SERVICE in ${app}. Exit code: ${serviceExitCode}" }

                            int encapsulatedExitCode = bat(returnStatus: true, script: """
                                @echo off
                                call "%GMU_HOME%\\GatewayMigrationUtility.bat" manageMappings -b "${currentBundlePath}" -t ENCAPSULATED_ASSERTION -a NewOrUpdate
                            """)
                            if (encapsulatedExitCode != 0) { error "Failed to apply NewOrUpdate mapping for ENCAPSULATED_ASSERTION in ${app}. Exit code: ${encapsulatedExitCode}" }

                            echo "Successfully prepared NewOrUpdate mappings for ${app}"
                        }
                    }
                }
            }
                ...
                withCredentials([usernamePassword(credentialsId: 'layer7-gateway-credentials', usernameVariable: 'GATEWAY_USERNAME', passwordVariable: 'GATEWAY_PASSWORD')]) {
                    script {
                        logReleaseApiMap.each { release, apps ->
                            apps.each { app ->
                                String currentBundlePath = "apis/${app}"
                                echo "Testing Layer7 migration for ${app}"
                                int exitCode = bat(returnStatus: true, script: """
                                    @echo off
                                    ...
                                    call "%GMU_HOME%\\GatewayMigrationUtility.bat" migrateIn ^
                                        ...
                                        --test
                                """)
                                if (exitCode != 0) { error "GMU test migration failed for API ${app} with error code ${exitCode}. Check results-${app}.xml" }
                                echo "GMU test migration successful for ${app}"
                            }
                        }
                    }
                }
                            ...
                            apps.each { app ->
                                ...
                                int exitCode = bat(returnStatus: true, script: """
                                    ...
                                    call "%GMU_HOME%\\GatewayMigrationUtility.bat" migrateIn ^
                                        ...
                                        --trustHostname
                                """)
                                if (exitCode != 0) { error "Deployment failed for API ${app} with error code ${exitCode}. Check gmu-results-${app}.xml" }
